...
# known solutions (in compressed form)
scripts = [
    (("NOT", "OR", "AND", "AND", "NOT", "AND"), "HCBAJD", "JJJJJJ"),
    (("OR", "AND", "NOT", "AND", "AND", "NOT", "OR"), "BCJDHAT", "JJJJJTJ"),
    (("OR", "AND", "AND", "NOT", "AND", "OR", "OR", "AND"), "ABCJDEHT", "JJJJJTTJ"),
]
...
for script in scripts:
	...
	script1 = ''
	for i in range(0, len(script[0])):
		script1 += (script[0][i] + ' ' + script[1][i] + ' ' + script[2][i] + '\n')
	script1 += 'RUN\n'
	#print(script1)
	inputs = [ord(x) for x in script1]
	outputs = runProgram(program, inputs)
	if outputs['output'][-1] > 255:
		print(outputs['output'][-1])
		break
	#else:
		#print(''.join([chr(x) for x in outputs['output']]))
