...
oxygenPath = area[oxygenPos]['path']
print(oxygenPath)

area = {}
area[oxygenPos] = {'type': 2, 'dist': 0, 'path': oxygenPath}  # types: 0 = wall, 1 = empty space, 2 = oxygen system; dist: distance from origin, path: shortest path to reach current point
curDist = 0
curNodes = [oxygenPos]
nextNodes = []
done = False
...
(latmin, latmax, longmin, longmax) = (999, -999, 999, -999)
for (lat1, long1) in area:
	latmin = min(latmin, lat1)
	latmax = max(latmax, lat1)
	longmin = min(longmin, long1)
	longmax = max(longmax, long1)
print({'lat': (latmin, latmax), 'long': (longmin, longmax)})
for i in range(latmax, latmin - 1, -1):
	line = ""
