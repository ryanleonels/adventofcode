...
for fileLine in fileLines:
	...
	for item in gift:
		if item in items:
			if item in ['cats', 'trees'] and items[item] <= gift[item]:
				match = False
				break
			if item in ['pomeranians', 'goldfish'] and items[item] >= gift[item]:
				match = False
				break
			if item not in ['cats', 'trees', 'pomeranians', 'goldfish'] and items[item] != gift[item]:
				match = False
				break
	if match == True:
		print(sue)
