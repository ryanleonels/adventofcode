...
i = 1000
while not done:
	adjGraph.add_edge(allDist[i][1], allDist[i][2], allDist[i][0])
	adjGraph.add_edge(allDist[i][2], allDist[i][1], allDist[i][0])
	distances = adjGraph.shortest_distances(0)
	if float("inf") not in distances.values():
		done = True
	else:
		i += 1
print(boxes[allDist[i][1]][0] * boxes[allDist[i][2]][0])
