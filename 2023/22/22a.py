...
nBricks = 0
for i in range(0, n):
	if [i] in supportList: # if a block is sole support of another block, start chain reaction
		supportList1 = []
		for j in range(0, n):
			supportList0 = []
			for k in range(0, len(supportList[j])):
				supportList0.append(supportList[j][k])
			supportList1.append(supportList0)
		nRemoved = 0
		bricksToRemove = [i]
		removed = []
		for j in range(0, n):
		...
		removed[i] = True
		while len(bricksToRemove) > 0:
			bricksToRemove1 = []
			for j in bricksToRemove: # remove and cause chain reaction
				removed[j] = True
				for k in range(0, n):
					if j in supportList1[k]:
						supportList1[k].remove(j)
			#print(supportList1)
			for j in range(0, n):
				if len(supportList[j]) > 0 and len(supportList1[j]) == 0 and removed[j] == False:
					bricksToRemove1.append(j)
			bricksToRemove = bricksToRemove1
			nRemoved += len(bricksToRemove)
			#print(bricksToRemove)
		nBricks += nRemoved
		#print(str(i) + " " + str(nRemoved))
print(nBricks)
